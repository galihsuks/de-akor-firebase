...
export default function Latihan5() {
    ...
    const [dataSubLatihan, setDataSubLatihan] = useState(
        currentUser.latihan > 6 ? 7 : currentUser.sublatihan
    );
    ...
    const handleUpdate = (e) => {
        e.preventDefault();
        fetchUpdateProgres(7, 1, true, currentUser.id);
    };
    const updateDataSub = () => {
        fetchUpdateProgres(6, sublatihan, false, currentUser.id);
        setCurrentUser((data) => ({
            ...data,
    ...
    if (page === 1)
        return (
            <div className="latihan-instruksi">
                <div>
                    <h1 style={{ color: "#0CCA98" }}>Latihan 5</h1>
