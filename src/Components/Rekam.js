...
export default function Rekam(props) {
    ...
    const highestFreq = useRef(0);
    const kesalahan = useRef(0);
    ...
    useEffect(() => {
        kesalahan.current = freqKePersentaseSalah(soal, highestFreq.current);
        stopRekam();
    ...
    function getPitch() {
        ...
        highestFreq.current = highestIndex.current * (24000 / spectrum.length);

        note.current = freqKePitch(highestFreq.current);
    ...
    return (
        <>
            ...
            {kesalahan.current && !intervalId.current ? (
                <>
                    {/* <p style={{ marginTop: "10px" }}>
                        Nada user yang terbaca:{" "}
                        {noteBuff.current.replace(/[0-9]/g, "")}
                    </p> */}
                    <p style={{ marginTop: "10px" }}>
                        Kesalahan: {kesalahan.current}
                    </p>
                </>
            ) : (
                ""
